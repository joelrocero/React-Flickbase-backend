...
router.route('/admin/add_articles').post(checkLoggedIn, grantAccess('createAny', 'article'), async (req, res) => {
  try {
...
router.route('/user/search').post(async (req, res) => {
  try {
    ...
    /// dog
    const re = new RegExp(`${req.body.keywords}`, 'gi')
    let aggQuery = Article.aggregate([{ $match: { status: 'public' } }, { $match: { title: { $regex: re } } }])
